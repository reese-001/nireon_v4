...
class CatalystMechanism(NireonBaseComponent):
    ...
    # FIX: Add **kwargs to the method signature to accept extra parameters like 'template_id'.
    async def _process_impl(self, data: Any, context: NireonExecutionContext, **kwargs) -> ProcessResult:
        template_id = kwargs.get('template_id')
        if template_id:
            context.logger.info(f"[{self.component_id}] Process called with template_id: {template_id}")

        if not isinstance(data, dict) or 'ideas' not in data or 'objective' not in data:
            # Check if we can adapt from a signal payload
            if isinstance(data, dict) and 'target_idea_id' in data: # Adapting from a Sentinel-like trigger
                 return ProcessResult(success=True, component_id=self.component_id, message="Catalyst received a trigger for a single idea, but requires a batch. This is a potential workflow improvement.", metadata={'status': 'noop_single_idea_trigger'})
        ...
        try:
            ...
            for idea in ideas_for_catalysis:
                ...
                prompt = build_prompt_for_idea(idea=idea, domain=domain, blend=blend, objective=objective,
                ...
                ce = CognitiveEvent(frame_id=current_frame.id, owning_agent_id=self.component_id,
                                    service_call_type='LLM_ASK', payload=llm_payload,
    ...
    async def _create_new_idea(self, original_idea: Idea, new_text: str, domain: str, blend: float, ctx: NireonExecutionContext) -> Tuple[Idea, float, float]:
        ...
        new_idea = Idea.create(text=new_text, parent_id=original_idea.idea_id, step=ctx.step, method=self.component_id, metadata=metadata)
    ...
    async def adapt(self, context: NireonExecutionContext) -> List[AdaptationAction]:
        ...
        if avg_sem_dist is not None and avg_sem_dist < self.catalyst_cfg.anti_constraints_diversity_threshold:
            actions.append(AdaptationAction(
                ...
                parameters={'config_key': 'blend_low', 'new_value': min(self.catalyst_cfg.blend_low + 0.05, self.catalyst_cfg.max_blend_low), 'old_value': self.catalyst_cfg.blend_low},
