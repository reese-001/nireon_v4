...
EXPLORER_METADATA = ComponentMetadata(
    ...
    produces=['IdeaGeneratedSignal', 'ExplorationCompleteSignal', 'FrameProcessingFailedSignal', 'TrustAssessmentSignal'], # Added TrustAssessmentSignal
...
class ExplorerMechanism(NireonBaseComponent):
    ...
    def __init__(self,
        ...
        # FIX: Add state to track idea assessments within a frame
        self._assessment_events: Dict[str, asyncio.Event] = {} # Key: idea_id, Value: Event
        self._frame_assessment_trackers: Dict[str, Dict[str, asyncio.Event]] = defaultdict(dict) # Key: frame_id, Value: {idea_id: Event}

        logger.info(
            f"ExplorerMechanism '{self.component_id}' (instance of {self.metadata.name} v{self.metadata.version}) created. "
            f"Gateway and FrameFactory will be resolved during initialization."
        )
    ...
    async def _initialize_impl(self, context: NireonExecutionContext) -> None:
        ...
        # FIX: Subscribe to TrustAssessmentSignal to know when downstream processing is done
    ...
    # FIX: Add handler for TrustAssessmentSignal
    async def _handle_trust_assessment(self, signal: TrustAssessmentSignal):
        """This method is called by the event bus when a trust assessment is completed."""
        idea_id = signal.payload.get('idea_id')
    ...
    async def _request_embedding_for_idea(self, frame: Frame, idea_text: str, idea_id: str,
        ...
        self._add_audit_log(frame, "EMBEDDING_REQUESTED", f"Embedding requested for idea {idea_id}.",
                            {'target_idea_id': idea_id, 'embedding_request_id': request_id})

        embedding_signal = EpistemicSignal(
            signal_type='EmbeddingRequestSignal',
            source_node_id=self.component_id,
            payload=embedding_request_payload,
            context_tags={'frame_id': frame.id}
        )
        await self.event_helper.publish_signal(embedding_signal, context)
        context.logger.info(
            f"[{self.component_id}] Requested embedding for idea '{idea_id}' (request_id: {request_id}) in frame '{frame.id}'.")
    ...
    async def _process_impl(self, data: Any, context: NireonExecutionContext) -> ProcessResult:
        ...
        frame_description = f"Exploration task initiated by '{self.component_id}' (exploration #{self._exploration_count}) for seed idea (hash: {input_hash}, preview: '{seed_input_text[:30]}...') using strategy '{self.cfg.exploration_strategy}'."
        ...
        audit_trail_entries: List[Dict[str, Any]] = [
            {"ts": datetime.now(timezone.utc).isoformat(), "event_type": "EXPLORER_TASK_STARTED",
             "summary": "Explorer task processing initiated."}
        ]
        context_tags = {
            ...
            'gateway_info': {'gateway_id': self.gateway.component_id if self.gateway else 'unknown',
        ...
        try:
            ...
            for i in range(num_variations_to_generate):
                ...
                llm_payload = LLMRequestPayload(prompt=prompt, stage=EpistemicStage.EXPLORATION, role='idea_generator', llm_settings=request_llm_settings)
                ce_llm_request = CognitiveEvent(frame_id=current_frame.id, owning_agent_id=self.component_id, service_call_type='LLM_ASK',
                                                payload=llm_payload, epistemic_intent='GENERATE_IDEA_VARIATION',
                ...
                self._add_audit_log(current_frame, "LLM_CE_CREATED", f"LLM_ASK CE created for attempt {i + 1}.",
                                    {'ce_id_related': ce_llm_request.event_id})

            # Process LLM responses and create ideas
            ...
            for i, resp_or_exc in enumerate(llm_responses):
                if isinstance(resp_or_exc, LLMResponse) and resp_or_exc.text and not resp_or_exc.get('error'):
                    ...
                    if len(variation_text) >= self.cfg.minimum_idea_length:
                        new_idea = self.event_helper.create_and_persist_idea(text=variation_text, parent_id=seed_input_id, context=context)
                        generated_idea_ids.append(new_idea.idea_id)
                        
                        assessment_event = asyncio.Event()
                        frame_tracker[new_idea.idea_id] = assessment_event

                        # FIX: Add the frame_id to the payload so the reactor rule can access it.
                        idea_signal_payload = {
                            'id': new_idea.idea_id,
                            'text': variation_text,
                            ...
                            'frame_id': current_frame.id, # <-- THIS IS THE FIX
                            'llm_response_metadata': {k: v for k, v in resp_or_exc.items() if k != 'text'}
                        }

                        idea_signal = IdeaGeneratedSignal(source_node_id=self.component_id, idea_id=new_idea.idea_id, idea_content=variation_text,
            ...
            return ProcessResult(success=True, component_id=self.component_id, output_data=exploration_complete_payload, message=message)
            ...
            return ProcessResult(success=False, component_id=self.component_id, message=f'Explorer critical error: {e}', error_code=str(ExplorerErrorCode.EXPLORER_PROCESSING_ERROR), metadata={'frame_id': current_frame_id_for_error})
    
    # ... The rest of the methods can remain as they are ...
    ...
    async def _generate_variations(self, seed_idea: str, context: NireonExecutionContext) -> List[str]:
        ...
        try:
            if self.cfg.exploration_strategy == 'depth_first':
                ...
                context.logger.warning(
                    f"Unknown or default strategy '{self.cfg.exploration_strategy}' - using simple variations.")
    ...
    async def _breadth_first_exploration(self, seed_idea: str, context: NireonExecutionContext) -> List[str]:
        ...
        while queue and len(variations) < max_vars_total:
            ...
            for i in range(self.cfg.max_variations_per_level):
                if len(variations) >= max_vars_total:
                    break
    ...
    async def react(self, context: NireonExecutionContext) -> List[SystemSignal]:
        ...
        # The logic for handling EmbeddingComputedSignal is not the primary issue, but we keep it.
        # Most of the race condition logic is now handled inside _process_impl.
        return []
    ...
    async def adapt(self, context: NireonExecutionContext) -> List[AdaptationAction]:
        ...
        if 'CONSIDER_INCREASING_DIVERGENCE_OR_REVISING_PROMPTS' in internal_recommendations:
            ...
            if current_strength < 0.8:
                ...
                action = AdaptationAction(action_type=AdaptationActionType.CONFIG_UPDATE,
                                          component_id=self.component_id,
                                          ...
                                          priority=2, estimated_impact='medium', requires_approval=True)
    ...
    async def health_check(self, context: NireonExecutionContext) -> ComponentHealth:
        ...
        status = 'HEALTHY'
        messages = []
        details = {'explorations_performed': self._exploration_count, 'configured_strategy': self.cfg.exploration_strategy}
        ...
        return ComponentHealth(component_id=self.component_id, status=status, message=final_message, details=details)