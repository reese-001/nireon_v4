...
class ExplorerEventHelper:
    ...
    def create_and_persist_idea(self, text: str, parent_id: Optional[str], context: NireonExecutionContext, metadata: Optional[Dict[str, Any]] = None) -> Idea:
        # The 'metadata' argument is now accepted and passed to the idea service
        idea = self._idea_service.create_idea(text=text, parent_id=parent_id, context=context, metadata=metadata)
        return idea
    ...
    async def publish_signal(self, signal_to_publish: EpistemicSignal, context: NireonExecutionContext) -> None:
        ...
        ce_custom_metadata = {
            ...
            'original_signal_id': signal_to_publish.signal_id
        }
        cognitive_event = CognitiveEvent(
            ...
            payload=ce_payload, 
            epistemic_intent=f'PUBLISH_{signal_to_publish.signal_type.upper()}', 
