...
class ExplorerConfig(BaseModel):
    ...
    exploration_strategy: Literal['depth_first', 'breadth_first', 'random'] = Field(default='depth_first', description='Strategy for exploring the idea space')
    ...
    creativity_factor: float = Field(default=0.7, ge=0.0, le=1.0, description='Creativity factor for exploration (0.0 = conservative, 1.0 = highly creative)')
    seed_randomness: Optional[int] = Field(default=None, description='Random seed for reproducible exploration (None = random)')
    ...
    exploration_timeout_seconds: float = Field(default=30.0, ge=1.0, le=300.0, description='Timeout for exploration operations in seconds')
