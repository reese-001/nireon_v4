...
class InitializationHelper:
    ...
    async def initialize_impl(self, context: NireonExecutionContext) -> None:
        ...
        try:
            ...
            self.sentinel.llm = self.sentinel.gateway
            ...
            for service_name in service_names_to_try:
                try:
                    ...
                    if isinstance(service_instance, IdeaService):
                        ...
                        context.logger.debug(f"[{self.sentinel.component_id}] Successfully acquired '{service_name}' (type: {type(service_instance)}).")
                        return True
                    elif service_instance is not None:
                        context.logger.warning(f"[{self.sentinel.component_id}] Found '{service_name}' but it is not an IdeaService instance (type: {type(service_instance)}).")
                except (KeyError, AttributeError):
                    context.logger.debug(f"[{self.sentinel.component_id}] Service '{service_name}' not found or error acquiring.")
                    continue
