    ...
    async def handle_react_phase(self, context: NireonExecutionContext) -> List[SystemSignal]:
        ...
        if assessment_data and isinstance(assessment_data, dict):
            ...
            if idea_id is not None:
                ...
                priority = 2 if is_stable else 4
                payload = {'idea_id': idea_id, 'is_stable': is_stable, 'trust_score': trust_score, 'rejection_reason': rejection_reason if not is_stable else None, 'full_assessment_summary': {k: v for k, v in assessment_data.items()}}
                msg = f"Idea '{idea_id}' assessed: {('Stable' if is_stable else 'Unstable')} (Trust: {trust_score:.2f})."
                if not is_stable and rejection_reason:
                    msg += f" Reason: {rejection_reason[:100]}{('...' if len(rejection_reason) > 100 else '')}"
                signals.append(SystemSignal(signal_type=signal_type, component_id=self.sentinel.component_id, message=msg, payload=payload, priority=priority))
        else:
            context.logger.debug(f"[{self.sentinel.component_id}] No 'last_sentinel_assessment_object' found in context for react phase.")
        return signals
    ...
    def adapt_weights(self, req_weights: List[float], ctx: NireonExecutionContext) -> Optional[AdaptationAction]:
        try:
            if not (isinstance(req_weights, list) and len(req_weights) == 3):
                ctx.logger.warning(f'[{self.sentinel.component_id}] adapt_weights: expected list[3], got {req_weights}')
                return None
            try:
                raw = np.asarray(req_weights, dtype=float)
            except ValueError:
                ctx.logger.warning(f'[{self.sentinel.component_id}] adapt_weights: non-numeric values {req_weights}')
                return None
            if (raw < 0).any():
                ctx.logger.warning(f'[{self.sentinel.component_id}] adapt_weights: negatives not allowed {req_weights}')
                return None
            old_live = self.sentinel.weights.tolist()
            self.sentinel.sentinel_cfg.weights = raw.tolist()
            ctx.logger.debug(f'[{self.sentinel.component_id}] adapt_weights: cfg updated to raw {raw.tolist()}')
            self.sentinel._init_helper.initialize_weights()
            new_live = self.sentinel.weights.tolist()
            if np.allclose(old_live, new_live):
                ctx.logger.debug(f'[{self.sentinel.component_id}] adapt_weights: no operational change (already {new_live})')
                return None
            params = {'parameter_name': 'axis_weights', 'old_operational_value': old_live, 'new_operational_value': new_live}
            ...
            return AdaptationAction(action_type=AdaptationActionType.PARAMETER_ADJUST, component_id=self.sentinel.component_id, description=msg, parameters=params)
        except Exception as e:
            ctx.logger.error(f'[{self.sentinel.component_id}] adapt_weights: unexpected error {e}', exc_info=True)
    ...
    def adapt_threshold(self, req_threshold: float, ctx: NireonExecutionContext) -> Optional[AdaptationAction]:
        try:
            if not 0.0 <= req_threshold <= 10.0:
                ctx.logger.warning(f'[{self.sentinel.component_id}] adapt_threshold: {req_threshold} outside 0-10')
                return None
            old_live = self.sentinel.trust_th
            if abs(req_threshold - old_live) < 1e-09:
                ctx.logger.debug(f'[{self.sentinel.component_id}] adapt_threshold: no change from live value {old_live}')
                return None
            self.sentinel.sentinel_cfg.trust_threshold = req_threshold
            self.sentinel.trust_th = req_threshold
            params = {'parameter_name': 'trust_threshold', 'old_value': old_live, 'new_value': req_threshold}
            ...
            ctx.logger.error(f'[{self.sentinel.component_id}] adapt_threshold: unexpected error {e}', exc_info=True)
            self.sentinel._error_count += 1
            return None