...
class MechanismGateway(NireonBaseComponent, MechanismGatewayPort):
    ...
    def __init__(self,
                 ...
                 metadata_definition: Optional[ComponentMetadata] = None) -> None:
    ...
    async def health_check(self, context: NireonExecutionContext) -> ComponentHealth:
        health = ComponentHealth(component_id=self.component_id,
                                 status='HEALTHY',
    ...
    async def ask_llm(self, ce: CognitiveEvent, context: NireonExecutionContext) -> LLMResponse:
        ...
        frame = await self._frame_factory.get_frame_by_id(context, ce.frame_id)
        if not frame:
            return LLMResponse({'text': f"Error: Frame '{ce.frame_id}' not found.",
                                'error': 'Frame not found', 'error_type': 'FrameNotFoundError', 'call_id': ce.event_id})
        ...
        # FIX: The context passed to the LLM call should be specific to that call, not the gateway's general context.
        # This prevents metadata from one component's context leaking into another's.
        ...
        try:
            ...
            llm_resp = LLMResponse({'text': f'Error processing LLM request: {exc}',
    ...
    def _resolve_from_registry(self, cls: type, existing: Optional[Any], context: NireonExecutionContext, log: logging.Logger):
    ...
    async def _handle_inactive_frame(self, ce: CognitiveEvent, frame: Frame, context: NireonExecutionContext) -> LLMResponse:
        ...
        resp = LLMResponse({'text': f'Error: {err}', 'error': err, 'error_type': 'FrameNotActiveError', 'call_id': ce.event_id})
    ...
    async def _handle_budget_exceeded(self, ce: CognitiveEvent, frame: Frame, exc: BudgetExceededError, context: NireonExecutionContext) -> LLMResponse:
        ...
        resp = LLMResponse({'text': f'Error: {msg}', 'error': str(exc), 'error_type': 'BudgetExceededError', 'call_id': ce.event_id,
