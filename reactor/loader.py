...
class RuleLoader:
    ...
    def load_rules_from_directory(self, directory_path: Path) -> List[ReactorRule]:
        ...
        # Search for .yml, .yaml, and .txt files to accommodate combined input files.
    ...
    def _load_rules_from_file(self, filepath: Path) -> None:
        try:
            ...
            if data.get('version', '1.0') != '1.0':
                logger.warning('Unexpected version in %s; continuing anyway', filepath.name)
            ...
            match act_type:
                ...
                case 'emit_signal':
                    if not act.get('signal_type'):
                        ...
                        return False
                case _:
                    logger.warning('Rule %s has unknown action type %s', rule_id, act_type)
        return True

    def _create_rule_from_config(self, cfg: Dict[str, Any]) -> Optional[ReactorRule]:
        signal_type = next((c.get('signal_type') for c in cfg['conditions'] if c.get('type') == 'signal_type_match'), None)
