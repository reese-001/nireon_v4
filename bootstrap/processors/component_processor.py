...
import asyncio  # kept for external callers that may await helpers
...
from bootstrap.health.reporter import (
    ComponentStatus,
    HealthReporter as BootstrapHealthReporter,
)
...
# --------------------------------------------------------------------------- #
# Optional dependency (merged‑config helper)
# --------------------------------------------------------------------------- #
...
try:
    ...
    class ConfigMerger:  # type: ignore
        @staticmethod
...
def _load_config(source: Any, component_id: str) -> Dict[str, Any]:
    """Return config dict from YAML path | inline dict | None."""
    if source is None:
        return {}
    ...
    if isinstance(source, Mapping):
        return dict(source)
    logger.warning(
        "Unexpected config type for '%s': %s – ignoring.",
        component_id,
        type(source),
    )
    return {}
...
def _apply_meta_override(
    base_meta: ComponentMetadata, overrides: Mapping[str, Any]
) -> ComponentMetadata:
    if not overrides:
        return base_meta
...
async def process_simple_component(
    ...
    """
    Instantiate & register a **simple component** declared in the manifest.

    Contract unchanged for callers; internals refactored to reduce duplication.
    """
    ...
    # ── Base metadata (default table → class override → fallback) ────────
    base_meta = (
        get_default_metadata(factory_key)
        if factory_key
        else _try_class_metadata(class_path)
    ...
    pydantic_defaults = (
        get_pydantic_defaults(cls_for_defaults, final_meta.name) if cls_for_defaults else {}
    )
    ...
    final_cfg = _merge_configs(pydantic_defaults, yaml_cfg, cfg_override, cid)
    ...
    try:
        ...
        health_reporter.add_component_status(
            cid, ComponentStatus.INSTANCE_REGISTERED, instance.metadata, []
        )
...
async def _instantiate_simple_component(
    ...
    """Create component instance (class or factory) – helper to keep main flow clean."""
    if class_path:
        cls_to_use = cls_for_defaults or import_by_path(class_path)
        if not issubclass(cls_to_use, NireonBaseComponent):
            raise BootstrapError(
                f"Class {class_path} for '{cid}' is not a NireonBaseComponent.",
                component_id=cid,
            )
        return await create_component_instance(
            cls_to_use, cfg, cid, metadata, common_deps=None
        )
    ...
    if factory_key and ctype == "mechanism" and mechanism_factory:
        ...
        if instance is None:
            raise BootstrapError(
                f"Factory returned None for '{cid}' / key '{factory_key}'",
                component_id=cid,
            )
        return instance

    raise BootstrapError(
        f"Cannot determine instantiation method for '{cid}'.",
        component_id=cid,
    )
...
async def _instantiate_shared_service(
    ...
    """Create shared‑service instance (supports BaseComponent & plain classes)."""
    deps = {"event_bus": event_bus, "registry": registry}

    if issubclass(service_class, NireonBaseComponent):
        return await create_component_instance(
            service_class, cfg, svc_id, meta, common_deps=deps
        )

    # Plain class – build kwargs dynamically
...
async def register_orchestration_command(
    ...
    """
    Register an orchestration command (class‑backed *or* metadata‑only).
    """
    ...
    try:
        ...
        _def_error(
            ...
            health_reporter,
            cmd_id,
            ComponentMetadata(id=cmd_id, name=cmd_id, category="orchestration_command"),
            strict,
        )
        return
...
# ===========================================================================
# Helper functions for status / error handling
# ===========================================================================


def _mark_disabled(
    reporter: BootstrapHealthReporter, comp_id: str | None, meta: ComponentMetadata
) -> None:
    reporter.add_component_status(
        comp_id or "unknown_disabled", ComponentStatus.DISABLED, meta, ["Disabled in manifest"]
    )
...
def _def_error(
    msg: str,
    reporter: BootstrapHealthReporter,
    comp_id: str | None,
    meta: ComponentMetadata,
    strict: bool,
    ...
    logger.error(msg)
    reporter.add_component_status(
        comp_id or "definition_error", ComponentStatus.DEFINITION_ERROR, meta, [msg]
    )
    if strict:
...
# ===========================================================================
# Existing helper (unchanged) – kept for compatibility
# ===========================================================================


def _build_component_metadata(
    base_metadata: ComponentMetadata, component_id_from_manifest: str, manifest_comp_definition: Dict[str, Any]
) -> ComponentMetadata:
    """
    Derive instance metadata from *base* + manifest overrides (unchanged signature).
    """
    if not isinstance(base_metadata, ComponentMetadata):
        raise TypeError(
            f"base_metadata for '{component_id_from_manifest}' must be ComponentMetadata, "
            f"got {type(base_metadata)}"
        )
    ...
    try:
        ...
        raise BootstrapError(
            f"Metadata construction error for '{component_id_from_manifest}': {exc}",
            component_id=component_id_from_manifest,
        ) from exc


def _apply_manifest_meta_fields(meta: Dict[str, Any], manifest: Dict[str, Any]) -> Dict[str, Any]:
    """Mutate & return *meta* with overrides from manifest."""
...
# ===========================================================================
# Port‑aware registry helper (signature unchanged)
# ===========================================================================
...
def _safe_register_service_instance_with_port(
    ...
    """Register `service_instance` with registry and (optionally) by port type."""
    if port_type:
        try:
