"""
Utility functions for the NIREON V4 bootstrap system.

This module provides common utilities for importing modules, loading YAML files,
detecting manifest types, and other helper functions used throughout bootstrap.
"""
...
def import_by_path(path: str, suppress_expected_errors: bool = True) -> Any:
    """
    Import a Python object by its full dotted path.
    
    Supports both 'module.attr' and 'module:attr' syntax for clarity.
    
    Args:
        path: Full dotted path like 'package.module.Class' or 'package.module:Class'
        suppress_expected_errors: If True, use DEBUG level for import errors
    ...
    # Parse path - support both 'module:attr' and 'module.attr' formats
    if ':' in path:
    ...
    if not module_name or not attr_name:
        logger.error(
            f"Could not parse module and attribute from path '{path}'. "
            f"Module: '{module_name}', Attribute: '{attr_name}'."
        )
        raise ValueError(f'Invalid import path format: {path}. Could not determine module and attribute.')
    
    # Set logging levels based on whether errors are expected
    ...
    try:
        ...
        logger.log(
            log_level_if_not_found,
            f"Failed to import module '{module_name}' from path '{path}': {e}",
            exc_info=log_level_if_not_found >= logging.ERROR
        )
    ...
    try:
        ...
        logger.log(
            log_level_if_not_found,
            f"Attribute '{attr_name}' not found in module '{module_name}' (from path '{path}'): {e}",
            exc_info=log_level_if_not_found >= logging.ERROR
        )
...
def load_yaml_robust(path: Union[str, Path, None]) -> Dict[str, Any]:
    """
    Load YAML file with robust error handling and placeholder detection.
    
    Args:
        path: Path to YAML file, can be None for empty result
        
    Returns:
        Dictionary from YAML file, or empty dict if file not found or invalid
    """
    if not path:
        return {}
    ...
    # Handle known placeholder patterns
    if path_str_representation == 'nireon/configs/default/components/{id}.yaml':
        ...
        return {}
    elif '{id}' in path_str_representation or '{ID}' in path_str_representation:
...
def detect_manifest_type(config_data: Dict[str, Any], style_hint: str = 'auto') -> str:
    """
    Detect the type of manifest based on its structure.
    
    Args:
        config_data: Parsed manifest data
        style_hint: Override hint ('auto', 'enhanced', 'simple')
        
    Returns:
        'enhanced' or 'simple' manifest type
    """
    if style_hint != 'auto':
        return style_hint
    
    # V4 enhanced manifest markers
    v4_enhanced_markers = {
        'version', 'metadata', 'shared_services', 'mechanisms', 
        'observers', 'managers', 'composites', 'orchestration_commands'
    }
    
    # Check for enhanced manifest
    if ('version' in config_data and 
        'metadata' in config_data and 
        any(marker in config_data for marker in v4_enhanced_markers - {'version', 'metadata'})):
        return 'enhanced'
    
    # Check for simple manifest
    if ('components' in config_data or 
        ('nireon_components' in config_data and 
         'mechanisms' in config_data.get('nireon_components', {}))):
        return 'simple'
...
def validate_config_hierarchy(config_dict: Dict[str, Any], config_name: str) -> bool:
    """
    Validate that a configuration dictionary follows expected structure.
...
def normalize_path(path: Union[str, Path], base_path: Union[str, Path, None] = None) -> Path:
    """
    Normalize a path and optionally resolve relative to a base path.
...
def safe_filename(name: str, max_length: int = 255) -> str:
    """
    Create a filesystem-safe filename from a component name.
...
def extract_class_name(class_path: str) -> str:
    """
    Extract the class name from a full dotted path.
    
    Args:
        class_path: Full path like 'package.module.ClassName'
...
def is_valid_component_id(component_id: str) -> bool:
    """
    Check if a component ID follows naming conventions.
