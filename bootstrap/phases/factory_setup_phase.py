...
class FactorySetupPhase(BootstrapPhase):
    ...
    async def _setup_budget_manager(self, context, correlation_id: str) -> ProcessResult:
        BUDGET_MANAGER_METADATA = ComponentMetadata(id='budget_manager_inmemory_instance', name='InMemoryBudgetManager Instance', version='1.0.0', category='service_core', description='In-memory budget manager for resource tracking (V2 enhanced)', requires_initialize=False)
        try:
            try:
            ...
            if hasattr(context, 'validation_data_store') and context.validation_data_store:
                context.validation_data_store.store_component_data(
                    component_id=BUDGET_MANAGER_METADATA.id,
                    original_metadata=BUDGET_MANAGER_METADATA,
                    resolved_config=budget_manager_config, # or budget_manager.config
                    manifest_spec={'class': f'{budget_manager.__class__.__module__}:{budget_manager.__class__.__name__}', 'config': budget_manager_config}
                )
            result = create_success_result(BUDGET_MANAGER_METADATA.id, f'{BUDGET_MANAGER_METADATA.name} created and registered with V2 context', output_data=budget_manager)
            result.set_correlation_id(correlation_id)
            result.add_metric('v2_context_enabled', budget_context is not None)
            logger.info(f'✓ {BUDGET_MANAGER_METADATA.name} created and registered with V2 integration')
            return result
        except Exception as e:
