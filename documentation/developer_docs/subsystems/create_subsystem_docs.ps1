<#
.SYNOPSIS
    Generates a detailed directory structure and rich README.md files for the NIREON V4 subsystems.

.DESCRIPTION
    This script reads a comprehensive, embedded YAML definition of the system's architecture.
    For each subsystem, it creates a folder and a detailed README.md file containing:
    - A detailed description of the subsystem's role.
    - A list of its public API/contracts.
    - A list of its downstream dependencies.
    - A Mermaid diagram visualizing its internal structure.

.NOTES
    Author: Nireon V4 Assistant
    Version: 2.0
    Requires: PowerShell 5.1+ and the 'powershell-yaml' module.

.EXAMPLE
    # Run from the .\docs\subsystem\ directory
    .\create_subsystem_docs.ps1
#>

# --- Pre-flight Checks ---
Write-Host "Starting NIREON V4 Subsystem Documentation Generator (v2.0)..." -ForegroundColor Cyan

if (-not (Get-Module -ListAvailable -Name powershell-yaml)) {
    Write-Host "WARNING: The 'powershell-yaml' module is required but not found." -ForegroundColor Yellow
    Write-Host "Please install it by running this command in an Administrator PowerShell prompt:" -ForegroundColor Yellow
    Write-Host "   Install-Module -Name powershell-yaml -Scope CurrentUser -Force" -ForegroundColor White
    exit 1
}
Import-Module powershell-yaml

# --- Embedded Subsystem README Content (YAML) ---
# This data block contains the complete, pre-generated content for each README file.
$yamlData = @'
Kernel:
  readme_content: |
    # Kernel Subsystem
...
Math_Engine:
  readme_content: |
    ...
    **Description:** A specialized subsystem for performing deterministic, symbolic mathematical computations. It is orchestrated by the `PrincipiaAgent`, which receives natural language queries, translates them into structured operations, and offloads the actual computation to a dedicated `MathPort` (currently implemented with a `SymPy` backend).

    ---

    ## Public API / Contracts

    - **`components.mechanisms.math_agents.principia_agent.PrincipiaAgent`**: The agent that receives `MathQuerySignal` and orchestrates the computation and explanation process.
    ...
    - **Accepted Signals:** `MathQuerySignal`
    - **Produced Signals:** `MathResultSignal`
    ...
    graph TD
        subgraph MathEngine [Math Engine]
            A[components/mechanisms/math_agents] --> B(principia_agent.py)
            C[infrastructure/math] --> D(sympy_adapter.py)
        end
    ```
Security_RBAC:
  readme_content: |
...
try {
    ...
    Write-Host "Base path for documentation set to: $($basePath.Path)" -ForegroundColor White

    # Parse the YAML data into a PowerShell object (hashtable)
    $subsystems = $yamlData | ConvertFrom-Yaml

    # Iterate over each subsystem defined in the YAML
    foreach ($subsystemName in $subsystems.Keys) {
        $subsystemInfo = $subsystems[$subsystemName]
        $subsystemDirectoryPath = Join-Path -Path $basePath -ChildPath $subsystemName

        # Create the subsystem directory if it doesn't exist
        if (-not (Test-Path -Path $subsystemDirectoryPath)) {
            New-Item -ItemType Directory -Path $subsystemDirectoryPath | Out-Null
            Write-Host "  Created directory: $($subsystemName)" -ForegroundColor Green
        } else {
            Write-Host "  Directory already exists: $($subsystemName)" -ForegroundColor Gray
        }

        # Get the pre-generated README content
        $readmeContent = $subsystemInfo.readme_content
        $readmeFilePath = Join-Path -Path $subsystemDirectoryPath -ChildPath "README.md"

        # Write the content to the README.md file
        Set-Content -Path $readmeFilePath -Value $readmeContent -Encoding UTF8
        Write-Host "    - Wrote README.md with detailed content." -ForegroundColor Gray
    }

    Write-Host "`nGeneration complete! All subsystem directories and detailed README files have been created." -ForegroundColor Green
