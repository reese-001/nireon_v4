...
# Import ComponentMetadata and ComponentRegistryMissingError from their new V4 location
...
class ComponentRegistry:
        ...
        with self._lock:
            if registration_key in self._components:
                ...
                if existing_instance is component_value:
                    ...
                    return
                else: # Detailed logging for re-registration
                    logger.debug(
    ...
    def register_service_instance(self, key: Union[str, Type], instance: Any) -> None:
        normalized_key = self.normalize_key(key)
        if not normalized_key.strip(): # Guard against empty keys
        ...
        with self._lock:
            ...
            # Metadata for service instances (not full components) might be handled by _safe_register_service_instance
    ...
    def get_service_instance(self, key: Union[str, Type]) -> Any:
        ...
        if normalized_key not in self._components:
            ...
            raise ComponentRegistryMissingError(
                str(key), 
                message=f"Service '{key}' (normalized: '{normalized_key}') not found in registry. Available components: {available_keys}"
            )
    ...
    def get_metadata(self, component_id: str) -> ComponentMetadata:
        ...
        raise ComponentRegistryMissingError(
            component_id, 
            message=f"Metadata for component '{component_id}' not found in registry. Components with metadata: {available_metadata}"
        )
    ...
    def register_certification(self, component_id: str, cert: Dict[str, Any]) -> None: # Retain
        ...
        with self._lock:
            if component_id not in self._components: # More nuanced logging
